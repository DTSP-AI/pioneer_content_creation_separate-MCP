...
Graph Flow:
    supervisor → content_creation → [tiktok, youtube_shorts] → END
"""
...
def build_content_workflow(
    ...
    This is the SINGLE graph orchestrator that coordinates all 4 agents:
    1. SupervisorAgent - Entry node, validates, routes
    2. ContentCreationAgent - Generates all content assets
    3. TikTokAgent - Publishes to TikTok (parallel)
    4. YouTubeShortsAgent - Publishes to YouTube Shorts (parallel)
    ...
    # Node 3: TikTokAgent (publishing)
    workflow.add_node("tiktok", tiktok_node)

    # Node 4: YouTubeShortsAgent (publishing)
    ...
    # Sequential flow: Supervisor → ContentCreation
    workflow.add_edge("supervisor", "content_creation")

    # Conditional routing after content creation
