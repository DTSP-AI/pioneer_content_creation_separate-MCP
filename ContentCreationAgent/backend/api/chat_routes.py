...
from backend.database.models import Thread, ThreadMessage, Agent, User, Tenant, WorkflowExecution
...
async def send_message(
    ...
    try:
        ...
        logger.info(f"DEBUG: agent_response keys = {list(agent_response.keys())}")
        logger.info(f"DEBUG: metadata value = {agent_response.get('metadata')}")
        logger.info(f"DEBUG: metadata type = {type(agent_response.get('metadata'))}")
        ...
        agent_message = ThreadMessage(
            ...
            message_metadata=agent_response.get("metadata")
        )

        # Log what we're about to save
        if agent_message.message_metadata:
            logger.info(f"DEBUG: Saving message with metadata keys: {list(agent_message.message_metadata.keys())}")
        else:
            logger.warning(f"DEBUG: No metadata to save for message")
...
async def approve_workflow(
    thread_id: str,
    message_id: str,
    background_tasks: BackgroundTasks,  # ✅ CRITICAL FIX: Add BackgroundTasks dependency
    session: AsyncSession = Depends(get_db_session)
):
    """
    Approve a workflow proposal and create the workflow.

    This endpoint now triggers actual workflow execution in the background.
    """
    try:
        ...
        # Create workflow from proposal and trigger execution
        workflow = await approve_workflow_proposal(
            thread_id=thread_id,
            message=message,
            session=session,
            background_tasks=background_tasks  # ✅ CRITICAL FIX: Pass BackgroundTasks
        )
