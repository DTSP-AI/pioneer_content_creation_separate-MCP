"""
PiAPI FastMCP Client

Connects to PiAPI FastMCP Server (TypeScript) via Server-Sent Events (SSE) to access
unified AI generation tools through the Model Context Protocol.

This implementation uses the official MCP SDK to establish a proper connection
and dynamically discover tools from the FastMCP server.

Architecture:
- Connects to FastMCP server (TypeScript) via SSE
- Uses MCP SDK for protocol-compliant communication
- Dynamically discovers available tools (4 unified tools) via MCP list_tools
- Converts MCP tools to LangChain-compatible tools
- Caches client connection for reuse

FastMCP Tools (4 unified):
- process_image_unified: Image generation, editing, enhancement
- generate_video_unified: Video generation, animation
- generate_audio_unified: Music and speech generation
...
class PiAPIMCPClient:
    """
    PiAPI FastMCP Client - Connects to FastMCP server (TypeScript) via MCP protocol.

    Manages connection lifecycle and tool retrieval from the FastMCP server
    using the official MCP SDK. FastMCP provides 4 unified tools for AI generation.

    Usage:
        async with PiAPIMCPClient() as client:
            tools = await client.get_tools()  # Returns 4 unified tools
    """
    ...
    def __init__(
        ...
        """
        Initialize PiAPI FastMCP client.

        Args:
            server_url: FastMCP server URL (defaults to config)

        Note:
            No API key needed here - the FastMCP server handles PiAPI authentication
            using credentials from PiAPI_MCP/piapi_fastmcp_server/.env.local
        """
    ...
    async def _maintain_connection(self) -> None:
        ...
        """
        try:
            logger.info(f"Connecting to FastMCP server: {self.server_url}/sse")

            # Use async with to properly manage SSE lifecycle
            async with sse_client(
                url=f"{self.server_url}/sse",  # FastMCP server serves SSE on /sse endpoint
                timeout=10.0,
    ...
    async def get_tools(self) -> List[Tool]:
        ...
        Returns:
            List of LangChain-compatible tools (expected: 4)
        ...
        try:
            logger.info("Discovering tools from FastMCP server via MCP protocol...")

            # Use MCP SDK to list available tools
            tools_response = await self.session.list_tools()

            logger.info(f"FastMCP server reported {len(tools_response.tools)} tools (expected: 4)")
...
async def get_piapi_mcp_client() -> Tuple[Optional[PiAPIMCPClient], List[Tool]]:
    """
    Get or create FastMCP client singleton (lazy initialization).

    Returns:
        Tuple of (client, tools) - tools list contains 4 unified FastMCP tools
    ...
    Note:
        ...
        - Returns 4 unified tools from FastMCP (TypeScript)
    ...
    try:
        ...
        logger.info(f"✅ FastMCP client initialized successfully with {len(tools)} tools (expected: 4)")

        return client, tools

    except asyncio.TimeoutError:
        logger.warning("⚠️ FastMCP initialization timed out after 15s - continuing without MCP tools")
        ...
        logger.error(f"❌ Unexpected error initializing FastMCP client: {e}")
        logger.exception("Full traceback:")
        logger.info("Application will use fallback video generation tools")
        return None, []
